  ...
  async function suggest() {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("ai-prompt-suggest", {
        body: { prompt_key: "action_plan", instruction, current_config: current },
      });
      if (error) throw error;
      if ((data as any)?.error) throw new Error((data as any).error);
    ...
    <div className="space-y-6">
      <Card>
        ...
        <Textarea rows={4} value={instruction} onChange={(e) => setInstruction(e.target.value)}
          placeholder="Ex.: reduzir o máximo de tarefas para 6, tornar o tom mais consultivo e reforçar critérios de conclusão." />
        ...
        <p className="mt-2 text-xs text-slate-500">A IA respeita regras invioláveis: sempre exige prazo, responsável (papel), métricas, medição de impacto e blocos obrigatórios.</p>
      </Card>
...
// ============================================================
// Histórico com compare A×B e restore
// ============================================================
