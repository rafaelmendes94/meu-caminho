...
import { Pencil, Eye, EyeOff, Archive, Copy, Trash2, PlayCircle, Cloud } from "lucide-react";
import { supabase } from "@/integrations/supabase/client";
import PlatformAdminLayout from "@/components/layouts/PlatformAdminLayout";
import { AudioForm, emptyAudio, type AudioItem } from "@/components/admin/AudioForm";

type Row = AudioItem & { id: string };

const KIND_LABEL: Record<string, string> = {
  pratica_guiada: "Prática", meditacao: "Meditação", respiracao: "Respiração",
  relaxamento: "Relaxamento", reflexao: "Reflexão", afirmacao: "Afirmação",
  audio_do_dia: "Áudio do dia", exercicio: "Exercício", foco: "Foco", outro: "Outro",
};
...
export default function PlatformContentAudiosScreen() {
  ...
  const load = async () => {
    ...
    let query = supabase.from("content_items").select("*").eq("type", "audio").order("updated_at", { ascending: false }).limit(500);
  ...
  const changeStatus = async (id: string, s: "draft" | "published" | "archived") => {
    if (s === "published") {
      ...
      if (row && !row.media_url) return toast.error("Envie um arquivo antes de publicar.");
    }
    ...
    toast.success(s === "published" ? "Áudio publicado." : s === "archived" ? "Áudio arquivado." : "Áudio despublicado.");
    void load();
  };
  const duplicate = async (r: Row) => {
    const { id, ...rest } = r as any;
    const copy = { ...rest, title: r.title + " (cópia)", slug: (r.slug ?? "audio") + "-copia-" + Date.now().toString(36), status: "draft", published_at: null, media_url: null };
    if (copy.metadata) copy.metadata = { ...copy.metadata, audio_path: null, audio_url: null };
    const { error } = await supabase.from("content_items").insert(copy);
    if (error) return toast.error(error.message);
    toast.success("Áudio duplicado."); void load();
  };
  const remove = async (id: string) => {
    if (!confirm("Excluir permanentemente? Isso NÃO remove o arquivo do R2.")) return;
    const { error } = await supabase.from("content_items").delete().eq("id", id);
    if (error) return toast.error(error.message);
    toast.success("Registro excluído."); void load();
  };
  ...
  return (
    <PlatformAdminLayout>
      <header className="flex justify-between items-start mb-6">
        <div>
          <p className="text-[10px] uppercase tracking-[0.25em] text-white/40">Content · Áudios</p>
          <h1 className="text-3xl font-black text-white mt-1">Áudios</h1>
          <p className="text-xs text-white/50 mt-1 max-w-xl">Práticas guiadas, meditação, respiração, foco e reflexão curta. Arquivos hospedados no Cloudflare R2.</p>
        </div>
        <div className="flex items-center gap-2">
          <Link to="/admin/storage/r2" className="px-3 py-2.5 border border-white/10 text-white/80 hover:bg-white/5 rounded-lg text-xs font-bold flex items-center gap-1.5">
            <Cloud className="w-3.5 h-3.5" /> Storage R2
          </Link>
          <button onClick={() => setEditing(emptyAudio())} className="px-4 py-2.5 bg-[#F88A2B] text-black rounded-lg text-sm font-bold">Novo áudio</button>
        </div>
      </header>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        ...
        <span className="ml-auto text-white/40 text-sm self-center">{filtered.length} áudios</span>
      </div>

      {loading && <p className="text-white/60 text-sm">Carregando…</p>}
      {error && <p className="text-red-400 text-sm">Erro: {error}</p>}
      {!loading && !error && filtered.length === 0 && <p className="text-white/40 text-sm">Sem áudios cadastrados.</p>}

      <div className="bg-white/5 border border-white/10 rounded-xl overflow-hidden">
        {filtered.map((r) => {
          const meta = (r.metadata ?? {}) as any;
          const kind = meta.audio_kind ?? "outro";
          const aiStatus = meta.ai_processing_status ?? "idle";
          const hasFile = !!r.media_url;
          return (
            <div key={r.id} className="flex items-center gap-3 px-4 py-3 border-b border-white/5">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-white/40 text-xs truncate">/{r.slug} · {r.duration_minutes ? `${r.duration_minutes}min` : "sem duração"}</p>
              </div>
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-white/10 text-white/70">{KIND_LABEL[kind] ?? kind}</span>
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${hasFile ? "bg-emerald-500/20 text-emerald-300" : "bg-white/10 text-white/40"}`}>
                {hasFile ? "R2 · OK" : "Sem arquivo"}
              </span>
      ...
      {editing && <AudioForm item={editing as AudioItem} onSaved={() => { setEditing(null); void load(); }} onClose={() => setEditing(null)} />}
          ...
          <div className="bg-white rounded-2xl overflow-hidden w-full max-w-lg" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-4">
              {previewing.media_url ? (
                <audio controls src={previewing.media_url} className="w-full" preload="metadata" />
              ) : (
                <p className="text-sm text-[#64748B]">Sem arquivo enviado.</p>
              )}
              <p className="mt-3 text-xs text-[#64748B]">{previewing.short_description}</p>
            </div>
          </div>
        </div>
      )}
