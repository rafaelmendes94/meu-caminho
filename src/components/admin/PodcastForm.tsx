...
export type PodcastFormat =
  | "video_podcast" | "audio_only" | "entrevista" | "conversa" | "reflexao_guiada";

export type PodcastProvider = "vturb" | "youtube" | "spotify" | "upload" | "external";

const FORMAT_LABELS: Record<PodcastFormat, string> = {
  video_podcast: "Podcast em vídeo",
  audio_only: "Somente áudio",
  entrevista: "Entrevista",
  conversa: "Conversa / bate-papo",
  reflexao_guiada: "Reflexão guiada",
};

const AUDIENCE_SUGGESTIONS = ["lideranca","gestor","alta-ansiedade","comunicacao","recuperacao","autoconhecimento","relacionamentos","todos"];
...
export const emptyPodcast = (): PodcastItem => ({
  ...
  media_url: "", file_url: null, is_premium: false, is_featured: false, published_at: null,
  metadata: {
    content_mode: "video_podcast",
    podcast_mode: "video_podcast",
    media_provider: "vturb",
    podcast_format: "video_podcast",
    series_name: "",
    season_number: null,
    episode_number: null,
    host_names: [],
  ...
  audience_tags: [], difficulty_level: 2, expected_outcomes: [],
  recommendation_weights: null,
});

export function PodcastForm({ item, onSaved, onClose }: { item: PodcastItem; onSaved: () => void; onClose: () => void }) {
  const [form, setForm] = useState<PodcastItem>(item);
  ...
  const provider: PodcastProvider = (meta.media_provider as PodcastProvider) ?? "vturb";
  const format: PodcastFormat = (meta.podcast_format as PodcastFormat) ?? "video_podcast";
  const scriptUrl: string = meta.vturb_script_url ?? (provider === "vturb" ? (form.media_url ?? "") : "");
  const audienceArr = form.audience_tags ?? [];
  const guests: string[] = Array.isArray(meta.guest_names) ? meta.guest_names : [];
  const hosts: string[] = Array.isArray(meta.host_names) ? meta.host_names : [];
  ...
  const set = <K extends keyof PodcastItem>(k: K, v: PodcastItem[K]) => setForm((f) => ({ ...f, [k]: v }));
  ...
  const validateForPublish = (): string | null => {
    ...
    if (!(meta.series_name ?? "").trim() && !(form.short_description ?? "").trim()) return "Série ou descrição curta obrigatória.";
    if (!format) return "Formato do podcast obrigatório.";
    if (!provider) return "Origem do conteúdo obrigatória.";
    if (provider === "vturb") {
      if (!vturbInfo.valid) return "URL VTurb inválida.";
    } else {
      const hasMedia = !!(form.media_url ?? "").trim() || !!(form.file_url ?? "").trim() || !!(meta.transcription ?? "").trim();
      if (!hasMedia) return "media_url válida ou transcrição/arquivo obrigatório.";
    }
    return null;
  };
  ...
  const save = async () => {
    ...
    setSaving(false);
    toast.success("Podcast salvo.");
    onSaved();
  };

  const analyze = async () => {
    if (!form.id) return toast.error("Salve o podcast antes de analisar com IA.");
    setAnalyzing(true);
    setMeta({ ai_processing_status: "processing", ai_processing_error: null });
    try {
      const { data, error } = await supabase.functions.invoke("process-podcast-video", {
        body: {
          podcast_id: form.id,
          media_url: provider === "vturb" ? scriptUrl : (form.media_url ?? ""),
    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-white border border-black/10 rounded-2xl p-6 w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-black text-[#0F172A]">{form.id ? "Editar" : "Novo"} episódio</h2>
          <span className="text-[10px] font-bold px-2 py-1 rounded-full bg-[#0F172A] text-white uppercase tracking-wider">Podcast</span>
        </div>

        <div className="grid grid-cols-2 gap-3">
          <label className="col-span-2 block"><span className="text-xs text-[#64748B]">Título do episódio *</span>
          ...
          <label className="col-span-2 block"><span className="text-xs text-[#64748B]">Série do podcast</span>
            <input value={meta.series_name ?? ""} onChange={(e) => setMeta({ series_name: e.target.value })} placeholder='Ex.: "Meu Caminho Cast"' className="w-full mt-1 px-3 py-2 bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg text-[#0F172A] text-sm" />
          </label>
          <label className="block"><span className="text-xs text-[#64748B]">Temporada</span>
            <input type="number" value={meta.season_number ?? ""} onChange={(e) => setMeta({ season_number: e.target.value ? Number(e.target.value) : null })} className="w-full mt-1 px-3 py-2 bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg text-[#0F172A] text-sm" />
          </label>
          <label className="block"><span className="text-xs text-[#64748B]">Nº do episódio</span>
            <input type="number" value={meta.episode_number ?? ""} onChange={(e) => setMeta({ episode_number: e.target.value ? Number(e.target.value) : null })} className="w-full mt-1 px-3 py-2 bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg text-[#0F172A] text-sm" />
          </label>

          <label className="block"><span className="text-xs text-[#64748B]">Formato *</span>
            <select value={format} onChange={(e) => setMeta({ podcast_format: e.target.value, podcast_mode: e.target.value === "audio_only" ? "audio_only" : "video_podcast" })} className="w-full mt-1 px-3 py-2 bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg text-[#0F172A] text-sm">
              {(Object.keys(FORMAT_LABELS) as PodcastFormat[]).map((k) => <option key={k} value={k}>{FORMAT_LABELS[k]}</option>)}
            </select>
          </label>
          <label className="block"><span className="text-xs text-[#64748B]">Origem *</span>
            <select value={provider} onChange={(e) => setMeta({ media_provider: e.target.value })} className="w-full mt-1 px-3 py-2 bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg text-[#0F172A] text-sm">
          ...
          <label className="col-span-2 block"><span className="text-xs text-[#64748B]">URL alternativa de áudio (mp3, YouTube, Spotify…)</span>
            <input value={meta.alt_audio_url ?? ""} onChange={(e) => setMeta({ alt_audio_url: e.target.value })} placeholder="https://…" className="w-full mt-1 px-3 py-2 bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg text-[#0F172A] text-sm" />
          </label>
          <div className="col-span-2">
            <StorageUpload bucket="content-audio" value={form.file_url ?? null} onChange={(v) => set("file_url", v)} label="Upload de arquivo (opcional)" />
          </div>
          ...
          {/* Apresentadores */}
          <div className="col-span-2">
            <span className="text-xs text-[#64748B]">Apresentador(es)</span>
            <div className="mt-1 flex flex-wrap gap-1.5 p-2 bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg min-h-[42px]">
              {hosts.map((h) => (
                <span key={h} className="text-xs px-2 py-1 rounded-full bg-[#0F172A] text-white flex items-center gap-1">
                  {h}
                  <button type="button" onClick={() => setMeta({ host_names: hosts.filter((x) => x !== h) })} className="text-white/70">×</button>
                </span>
              ))}
              <input value={newHost} onChange={(e) => setNewHost(e.target.value)}
                onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); const v = newHost.trim(); if (v && !hosts.includes(v)) { setMeta({ host_names: [...hosts, v] }); setNewHost(""); } } }}
                placeholder="Enter para adicionar" className="flex-1 min-w-[140px] bg-transparent text-xs text-[#0F172A] outline-none px-1" />
            </div>
          </div>
          {/* Convidados */}
          <div className="col-span-2">
            <span className="text-xs text-[#64748B]">Convidado(s)</span>
            <div className="mt-1 flex flex-wrap gap-1.5 p-2 bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg min-h-[42px]">
              {guests.map((g) => (
                <span key={g} className="text-xs px-2 py-1 rounded-full bg-[#F88A2B] text-black flex items-center gap-1">
                  {g}
                  <button type="button" onClick={() => setMeta({ guest_names: guests.filter((x) => x !== g) })} className="text-black/70">×</button>
                </span>
              ))}
              <input value={newGuest} onChange={(e) => setNewGuest(e.target.value)}
                onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); const v = newGuest.trim(); if (v && !guests.includes(v)) { setMeta({ guest_names: [...guests, v] }); setNewGuest(""); } } }}
                placeholder="Enter para adicionar" className="flex-1 min-w-[140px] bg-transparent text-xs text-[#0F172A] outline-none px-1" />
            </div>
          </div>
          ...
          <label className="block"><span className="text-xs text-[#64748B]">CTA · texto do botão</span>
            <input value={meta.cta_label ?? ""} onChange={(e) => setMeta({ cta_label: e.target.value })} placeholder="Ex.: Continuar reflexão" className="w-full mt-1 px-3 py-2 bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg text-[#0F172A] text-sm" />
          </label>
          ...
          {/* Transcrição / notas */}
          <label className="col-span-2 block"><span className="text-xs text-[#64748B]">Transcrição (obrigatória para análise IA profunda)</span>
          ...
          <label className="col-span-2 block"><span className="text-xs text-[#64748B]">Notas para a IA (contexto, tom, convidados, temas)</span>
          ...
          <div className="col-span-2 rounded-lg border border-[#E2E8F0] bg-[#F8FAFC] p-3 flex items-center justify-between gap-3">
            ...
            <button type="button" onClick={analyze} disabled={analyzing || !form.id} className="px-4 py-2 bg-[#0F172A] text-white text-xs font-bold rounded-lg disabled:opacity-40">
              {analyzing ? "Analisando…" : "Analisar podcast com IA"}
            </button>
          </div>
          ...
          {meta.ai_summary && (
            <div className="col-span-2 rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-xs text-[#064E3B]">
              ...
              {Array.isArray(meta.key_moments) && meta.key_moments.length > 0 && (
                <div className="mt-2">
                  <b>Momentos-chave:</b>
                  <ul className="list-disc pl-5">
                    {meta.key_moments.slice(0, 8).map((m: any, i: number) => (
                      <li key={i}>{typeof m === "string" ? m : `${m.timestamp ?? ""} — ${m.label ?? m.title ?? ""}`}</li>
                    ))}
                  </ul>
                </div>
              )}
              {Array.isArray(meta.reflection_questions) && meta.reflection_questions.length > 0 && (
                <p className="mt-2"><b>Perguntas de reflexão:</b> {meta.reflection_questions.join(" · ")}</p>
              )}
              {Array.isArray(meta.ideal_audience) && meta.ideal_audience.length > 0 && (
                <p><b>Público ideal:</b> {meta.ideal_audience.join(" · ")}</p>
              )}
              {meta.emotional_depth && <p><b>Profundidade emocional:</b> {meta.emotional_depth}</p>}
            </div>
          )}
          ...
          {/* Autores + tags */}
          <div className="col-span-2">
            <span className="text-xs text-[#64748B]">Autor(es) vinculado(s) do CMS</span>
